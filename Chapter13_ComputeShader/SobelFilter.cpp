...
{
	m_hCpuSrv = hCpuDescriptor;
	m_hCpuUav = hCpuDescriptor.Offset(1, descriptorSize);
	m_hGpuSrv = hGpuDescriptor;
	m_hGpuUav = hGpuDescriptor.Offset(1, descriptorSize);
	BuildDescriptors();
}
...
void SobelFilter::OnProcess(ID3D12GraphicsCommandList* cmdList, ID3D12RootSignature* rootSig, ID3D12PipelineState* pso, CD3DX12_GPU_DESCRIPTOR_HANDLE input)
{
	cmdList->SetComputeRootSignature(rootSig);
	cmdList->SetPipelineState(pso);

	cmdList->SetComputeRootDescriptorTable(0, input);
	cmdList->SetComputeRootDescriptorTable(2, m_hGpuUav);

	cmdList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(m_Output.Get(),
		D3D12_RESOURCE_STATE_GENERIC_READ, D3D12_RESOURCE_STATE_UNORDERED_ACCESS));

	UINT numGroupX = (UINT)ceilf(m_uiWidth / 16.0f);
	UINT numGroupY = (UINT)ceilf(m_uiHeight / 16.0f);
