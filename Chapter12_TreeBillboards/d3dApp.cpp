...
void D3DApp::CreateMsaaResource()
{
	// 创建渲染目标 和 深度/模板描述符堆
	...
	HR(m_pd3dDevice->CreateDescriptorHeap(&msaaRTVHeapDesc, IID_PPV_ARGS(m_MsaaRTVHeap.GetAddressOf())));
	...
	HR(m_pd3dDevice->CreateDescriptorHeap(&msaaDSVHeapDesc, IID_PPV_ARGS(m_MsaaDSVHeap.GetAddressOf())));

	// 创建带MSAA的Render Target
	D3D12_RESOURCE_DESC msaaRTDesc = CD3DX12_RESOURCE_DESC::Tex2D(
		m_BackBufferFormat, m_ClientWidth, m_ClientHeight, 1, 1, 4);
	...
	m_pd3dDevice->CreateRenderTargetView(m_MsaaRenderTarget.Get(),
		&msaaRTVDesc, m_MsaaRTVHeap->GetCPUDescriptorHandleForHeapStart());

	// 创建带MSAA的深度/模板资源和视图
	D3D12_RESOURCE_DESC msaaDSDesc = CD3DX12_RESOURCE_DESC::Tex2D(
		m_DepthStencilFormat, m_ClientWidth, m_ClientHeight, 1, 1, 4);
	msaaDSDesc.Flags |= D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL;

	D3D12_CLEAR_VALUE msaadepthClearValue = {};
	msaadepthClearValue.Format = m_DepthStencilFormat;
	msaadepthClearValue.DepthStencil.Depth = 1.0f;
	msaadepthClearValue.DepthStencil.Stencil = 0;

	HR(m_pd3dDevice->CreateCommittedResource(
		&CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT),
		D3D12_HEAP_FLAG_NONE, &msaaDSDesc,
		D3D12_RESOURCE_STATE_DEPTH_WRITE,
		&msaadepthClearValue,
		IID_PPV_ARGS(m_MsaaDepthStencil.ReleaseAndGetAddressOf())));

	D3D12_DEPTH_STENCIL_VIEW_DESC msaaDSVVDesc;
	msaaDSVVDesc.Format = m_DepthStencilFormat;
	msaaDSVVDesc.ViewDimension = D3D12_DSV_DIMENSION_TEXTURE2DMS;
	msaaDSVVDesc.Flags = D3D12_DSV_FLAG_NONE;
	m_pd3dDevice->CreateDepthStencilView(m_MsaaDepthStencil.Get(),
		&msaaDSVVDesc, m_MsaaDSVHeap->GetCPUDescriptorHandleForHeapStart());
}
