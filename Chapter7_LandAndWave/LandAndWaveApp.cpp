...
void LandAndWaveApp::BuildFrameResources()
{
	for(int i = 0;i<g_numFrameResources;++i)
	{
		m_FrameResources.push_back(std::make_unique<FrameResource>(m_pd3dDevice.Get(),
			1, (UINT)m_AllRitems.size(), m_Waves->GetVertexCount()));
	}
}
...
float LandAndWaveApp::GetHillsHeight(float x, float z) const
{
	return 0.3f * (z * sinf(0.1f * x) + x * cosf(0.1f * z));
}

XMFLOAT3 LandAndWaveApp::GetHillsNormal(float x, float z) const
{
	XMFLOAT3 N(
